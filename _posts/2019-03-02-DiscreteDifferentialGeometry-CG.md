...
#### 曲面的微分（切向量）
我们关心曲面映射的微分$$df$$。$$df$$将$$M$$上的某一点$$p$$处的切向量$$X$$变换到实数空间中的$$df(X)$$。我们之所以关心微分（differential）而不是这个映射本身，是因为直接刻画$$f$$可能是一件很复杂的事情，因为$$M$$是可以弯曲的。换言之，我们更关心$$M$$上面的某一个切向量（相当于先将M在某个点附近用欧氏空间近似，再研究该点附近的一个向量）会如何随着$$f$$的改变而改变——原空间$$M$$中的一个向量$$X$$，在$$f$$使得$$M$$变为$$f(M)$$的同时，随之变为$$df(X)$$。换言之，微分$$df$$刻画了**如何将一个向量从某个空间变换到另一个空间**。至于为什么是切向量，可以参照[An Introduction to Manifold](http://im0.p.lodz.pl/~kubarski/AnalizaIV/Wyklady/L-Tu-1441973990.pdf)的第二章，该章节系统地证明了欧氏空间中的切向量与导数的等价性。换言之，因为流形在局部的性质很好，所以我们可以用线性空间的方法去研究它。表达这样一个关系最重要的式子，就是$$T_p(\mathbb{R}^n) \cong \mathcal{D}_p(\mathbb{R}^n)$$，它表示点$$p$$的切空间与该点处的导数空间是等价的，从而有：

$$
v = \sum v^i e_i = \left. \sum v^i \frac{\partial}{\partial x^i} \right\rvert_p
$$

其中$$e_i$$指的是切空间中的基，而$$\frac{\partial}{\partial x^i} \rvert_p$$指的是欧氏空间中$$p$$点的偏微分，二者是等价的。

值得注意的是，如果我们考虑的不是单个向量，而是一个向量场，那么$$df$$同样可以将整个向量场从某个空间变换到另一个空间。我们定义一个$$\mathbb{R}^n$$中的一个开子集$$U$$上的向量场为：

$$
X_p = \left. \sum a^i(p) \frac{\partial}{\partial x^i} \right\rvert_p, p \in U, a^i(p) \in \mathbb{R}
$$

注意我们重复使用了$$X$$这个符号，在前面它表示的是一个向量，这里表示的是一个向量场函数。我们将$$X$$所处的空间$$T_pM$$定义为切空间，即$$M$$的所有切向量的表示。我们同样可以用导数的方法来理解向量场，假定对于开集$$U$$上有连续函数$$g$$，那么我们可以定义一个新的函数$$Xg(p)$$如下：

$$(Xg)(p) = X_pg = \sum a^i(p)\frac{\partial g}{\partial x^i}(p)$$
这就是向量场的导数表示。
...
对于一个实值函数而言，我们经常将它画成下图左边的样子，即$$\phi(x)$$随着$$x$$的变化而变化的一张函数图像。事实上，从微分几何的视角来看，我们可以认为$$x$$所在的空间为$$M$$（这里$$M$$也是$$\mathbb{R}^1$$），而 $$ \phi(x) $$所在的空间为$$\mathbb{R}^1$$。从这个角度看，在M空间中的切向量$$X$$经过$$\phi$$变换后所对应的变化$$d\phi(X)$$恰好就是$$\phi'$$，即$$\phi$$的导数。我们将这个关系写作：

$$
    \phi' = d\phi(X)
$$

这就将几何上的微分算子和欧氏空间中的导数联系了起来，当然了，这里的流形$$M$$本身也是一个欧氏空间，因此这个对应是很简单也很直接的。我们用这个例子只是为了直观地展示一下微分究竟做了一件什么事情。
